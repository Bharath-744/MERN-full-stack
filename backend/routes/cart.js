...
router.post("/add", isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    await cart.save();
    cart = await Cart.findOne({ user: req.userId }).populate("items.product"); // ensure populated
...
router.put("/", isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    if (!product) {
      ...
      await cart.save();
      return res.status(404).json({ message: "Product no longer exists. Removed from cart.", cart });
    }
